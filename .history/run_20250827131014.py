...
def export_payment_csv():
    ...
    writer.writerow(['Date', 'Trips', 'Fare Collected (KES)', 'Commission (KES)', 'Status'])
    for p in payments:
        writer.writerow([p.payment_date, p.total_trips, p.total_fare_collected, p.commission_earned, p.payment_status])
...
def staff_management():
    ...
    staff_members = User.query.filter(User.role.in_(['employee', 'admin'])).all()
...
def edit_staff(staff_id):
    ...
    if request.method == 'POST':
        ...
        staff.username = request.form.get('staff_id')
        staff.role = request.form.get('role')
        staff.email = request.form.get('contact')
        staff.status = request.form.get('status')

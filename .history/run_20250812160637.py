...
def confirmation():
    # Get the most recent booking for the current user or session
    if current_user.is_authenticated:
        booking = Booking.query.filter_by(user_id=current_user.id).order_by(Booking.id.desc()).first()
    else:
        # For anonymous users, we might need to use session or other methods to track the booking
        # For now, we'll just get the most recent booking
    ...
    return render_template("confirmation.html", booking=booking, route=route)
...
def passenger_dashboard():
    ...
    # Calculate total fare (assuming a base fare calculation)
    # Since there's no fare field in Booking model, we'll calculate based on routes
    total_fare = 0
    for booking in passenger_bookings:
...
def performance_tracker():
    ...
    table_rows = []
    for p in performances:
        # Get route distance for fare calculation
        route = Route.query.filter_by(route_name=p.route).first()
        calculated_fare = 0
        if route and route.distance:
            calculated_fare = route.distance * 20 * p.trips
        else:
            # Fallback calculation if distance is not available
            calculated_fare = p.fare_collected
...
def admin_dashboard():
    ...
    for perf in recent_performances:
        ...
        # Get route distance for fare calculation
        route = Route.query.filter_by(route_name=perf.route).first()
        calculated_fare = 0
        if route and route.distance:
            calculated_fare = route.distance * 20
        else:
            # Fallback calculation if distance is not available
            calculated_fare = perf.fare_collected
            
        recent_activity.append({
            'type': 'performance',
            'date': perf.date.strftime('%Y-%m-%d') if perf.date else "N/A",
            'description': f"Performance: {employee_name} collected KES {calculated_fare}"
        })
...
def reports():
    ...
    for perf in recent_performances:
        ...
        recent_activities.append({
            ...
            'details': f"KES {calculated_fare:,.2f} on Route: {perf.route}"
        })

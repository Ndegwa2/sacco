...
def employee_dashboard():
    ...
    performances = Performance.query.filter_by(employee_id=current_user.id).all()
    total_trips = sum(p.trips for p in performances) if performances else 0
    total_earnings = sum(p.commission for p in performances) if performances else 0
    
    # Get employee's driver logs
...
def passenger_dashboard():
    ...
    passenger_bookings = Booking.query.filter_by(name=current_user.full_name).all()
    ...
    recent_bookings = Booking.query.filter_by(name=current_user.full_name).order_by(Booking.id.desc()).limit(5).all()
    
    return render_template("passenger_dashboard.html",
                         passenger_name=current_user.full_name or current_user.username,
...
def admin_dashboard():
    ...
    # This is a simple sort that works with string dates
...
def reports():
    ...
    active_vehicles_count = Fleet.query.filter_by(status='active').count()
    ...
    recent_fleet = Fleet.query.order_by(Fleet.id.desc()).limit(3).all()

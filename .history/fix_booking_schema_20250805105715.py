    ...
    with app.app_context():
        try:
            ...
            if 'sqlite' in str(db.engine.url):
                ...
                result = connection.execute("PRAGMA table_info(booking)")
                columns = [row[1] for row in result.fetchall()]
                
                if 'user_id' not in columns:
                    print("➕ Adding user_id column...")
                    connection.execute("ALTER TABLE booking ADD COLUMN user_id INTEGER")
                ...
                if 'created_at' not in columns:
                    ...
                    connection.execute("ALTER TABLE booking ADD COLUMN created_at DATETIME")

...
def view_payment_summary():
    ...
    assignments = AssignedRoute.query.filter_by(employee_id=employee_id).all()
    num_routes_assigned = len(assignments)

    # Assuming each booking is worth $10
    bookings = Booking.query.filter_by(name=current_user.full_name).all()
    num_bookings_completed = len(bookings)
    earnings_per_booking = 10
    total_earnings = num_bookings_completed * earnings_per_booking

    # Assuming 10% deductions
    deductions = total_earnings * 0.1
    net_pay = total_earnings - deductions

    payment_summary = {
        'employee_id': employee_id,
        'num_routes_assigned': num_routes_assigned,
        'num_bookings_completed': num_bookings_completed,
